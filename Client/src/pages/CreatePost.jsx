...
const CreatePost = () => {
  ...
  const generateImage = async () => {
    if(form.prompt){
      try {
        ...
        const response = await fetch('http://localhost:3000/api/v1/dalle/ai',{
          method:"POST",
          headers:{
        ...
        const photo = `data:image/jpeg;base64,${data.photo}`;
      ...
      alert('pleae enter a prompt Here');
    }
  }
  ...
  const handelSubmit = async (e) => {
    ...
    if(form.photo && form.prompt){
      ...
      alert("Please Enter Your Prompt And Photo");
    }
  }
    ...
    <section className='max-w-7xl mx-auto'>
      <div>
        ...
        <p className='mt-2 text-[#666e75] text-[16px] max-w-[500px]'>
          Create imaginative and visually stunning images through by DALL-E AI and shear them with the communty
        </p>
      </div>
      ...
      <form className='mt-16 max-w-3xl' onSubmit={handelSubmit}>
        ...
        <div className='mt-10'>
          <p className='mt-2 text-[#666e75] text-[14px]'>Once You have created the image your want , you can shear it with others in the community</p>
        </div>
        <button
          type='submit'
          className='mt-3 text-white bg-[#6469ff] font-medium rounded-md text-sm w-full sm:w-auto px-5 py-2.5'
        >
          {loading ? 'Sharing...' : 'Shear With The Community'}
        </button>
      </form>
    </section>
  )
